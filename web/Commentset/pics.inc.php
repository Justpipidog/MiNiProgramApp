...
global $_GPC, $_W;
        ...
        if ($opt == 'display'){
            ...
            $total = pdo_fetchcolumn("SELECT count(*) FROM ".tablename('sudu8_page_products')."as i left join" .tablename('sudu8_page_cate')." as c on i.cid = c.id WHERE i.uniacid = ".$uniacid." and i.type ='showPic'");
            $pageindex = max(1, intval($_GPC['page']));
            $pagesize = 10;  
            $start = ($pageindex-1) * $pagesize;
            $pager = pagination($total, $pageindex, $pagesize);  
            $products = pdo_fetchall("SELECT i.num,i.thumb,i.title,i.id,c.name,i.type FROM ".tablename('sudu8_page_products')."as i left join" .tablename('sudu8_page_cate')." as c on i.cid = c.id WHERE i.uniacid = ".$uniacid." and i.type ='showPic' ORDER BY i.num DESC,i.id DESC LIMIT ".$start.",".$pagesize);
            //var_dump($products);
            // 获取文章分类
