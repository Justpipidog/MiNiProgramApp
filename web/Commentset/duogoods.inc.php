...
if ($opt == 'display') {
    $total = pdo_fetchcolumn("SELECT count(*) FROM ".tablename('sudu8_page_products') ." as a LEFT JOIN ".tablename('sudu8_page_cate')." as b on a.cid = b.id WHERE a.uniacid = :uniacid and a.is_more = 3 order by a.num desc,a.id desc", array(':uniacid' => $uniacid));
    $pageindex = max(1, intval($_GPC['page']));
    $pagesize = 10;  
    $start = ($pageindex-1) * $pagesize;
    $pager = pagination($total, $pageindex, $pagesize);
    $products = pdo_fetchall("SELECT a.*,b.name as cname FROM ".tablename('sudu8_page_products') ." as a LEFT JOIN ".tablename('sudu8_page_cate')." as b on a.cid = b.id WHERE a.uniacid = :uniacid and a.is_more = 3 order by a.num desc,a.id desc LIMIT ".$start.",".$pagesize, array(':uniacid' => $uniacid));
