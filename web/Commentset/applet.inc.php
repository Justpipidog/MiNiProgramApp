...
load()->func('tpl');
 global $_GPC, $_W;
        ...
        if ($opt == 'display'){
            $_W['page']['title'] = '小程序管理';
            $total = pdo_fetchcolumn("SELECT count(*) FROM ".tablename('sudu8_page_wxapps')."as i left join" .tablename('sudu8_page_cate')." as c on i.cid = c.id WHERE i.uniacid = ".$uniacid." and i.type ='showWxapps'");
            $pageindex = max(1, intval($_GPC['page']));
            $pagesize = 1;  
            $start = ($pageindex-1) * $pagesize;
            $pager = pagination($total, $pageindex, $pagesize);
            $wxapps = pdo_fetchall("SELECT i.num,i.thumb,i.title,i.id,c.name,i.type FROM ".tablename('sudu8_page_wxapps')."as i left join" .tablename('sudu8_page_cate')." as c on i.cid = c.id WHERE i.uniacid = ".$uniacid." and i.type ='showWxapps' ORDER BY i.num DESC,i.id DESC LIMIT ".$start.",".$pagesize);
            // 获取文章分类
        ...
        if ($opt == 'post'){
            ...
            if (checksubmit('submit')) {
                ...
                message('小程序 添加/修改 成功!', $this->createWebUrl('Commentset', array('op'=>'applet','opt'=>$opt,'cateid'=>$_GPC['cateid'],'chid'=>$_GPC['chid'],'id'=>$id)), 'success');
