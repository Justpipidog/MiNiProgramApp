<?php 
	...
	if($act == "display"){
	    ...
	    $cateid = $_GPC['cateid'];
	    $chid = $_GPC['chid'];
        $total = pdo_fetchcolumn("SELECT count(*) FROM ".tablename('sudu8_page_usergroup')." WHERE uniacid = :uniacid", array(':uniacid' => $uniacid));
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = 10;  
        $start = ($pageindex-1) * $pagesize;
        $pager = pagination($total, $pageindex, $pagesize); 
		$item = pdo_fetchall("SELECT * FROM ".tablename('sudu8_page_usergroup') ." WHERE uniacid = :uniacid ORDER BY id DESC LIMIT ".$start.",".$pagesize, array(':uniacid' => $uniacid));
	...
	if($act == 'update'){
	    ...
	    if(empty($group)){
	    	...
	    	$res = pdo_update("sudu8_page_usergroup", array("name" => $name, "remark" => $remark), array("id" => $id, "uniacid" => $uniacid));
	    }
	    if($res){
